            ...
            <div class="boxed-slider owl-carousel owl-carousel-lazy owl-theme owl-theme-light">
                @foreach ($offers as $offer)
                    <div class="boxed-slide boxed-slide-1">
                        <figure>
                            <img class="slide-bg owl-lazy" data-src="{{asset('storage/app/public/images/'. $offer->image)}}" src="{{asset('storage/app/public/images/'. $offer->image)}}" alt="banner" width="870" height="300">
                        </figure>
                        <div class="slide-content">
                            <h4>{{$offer->title}}</h4>
                            <a href="{{ route('offers',str_replace(' ', '-', $offer->title) ) }}" class="btn btn-sm btn-primary">Learn More</a>
                        </div>
                    </div> 
                @endforeach
                        ...
                        <div class="widget-body">
                            <ul class="cat-list">
                                @foreach ($categories as $cat)
                                  <li><a href="{{route('products', ['cat'=> $cat->category_name, 'brand' => 'all', 'st' => 'desc', 'pg' => 10, 'search'=> 'query'])}}">{{$cat->category_name}}</a></li>  
                                @endforeach
                            </ul>
                        </div>
                        <!-- End .widget-body -->
                        ...
                        <div class="pb-0 widget-body">
                            <ul class="cat-list">
                                @foreach ($brands as $brand)
                                  <li><a href="{{route('products', ['cat'=> 'all', 'brand' => $brand->brand_name , 'st' => 'desc', 'pg' => 10, 'search'=> 'query'])}}">{{$brand->brand_name}}</a></li>  
                                @endforeach
                            </ul>
                        </div>
                        <!-- End .widget-body -->
                                ...
                                <div class="product-default left-details product-widget">
                                    <figure>
                                        <a href="{{ route('productsingle',str_replace(' ', '-', $feat->name) ) }}">
                                            <img src="{{asset('storage/app/public/images/'. $feat->featured_image)}}" width="75" height="75" alt="product" />
                                            <img src="{{asset('storage/app/public/images/'. $feat->featured_image)}}" width="75" height="75" alt="product" />
                                        </a>
                                    </figure>
